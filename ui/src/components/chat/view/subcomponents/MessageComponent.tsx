...
const MessageComponent = memo(({ message, prevMessage, createDiff, onFileOpen, onShowSettings, onGrantSessionToolPermission, autoExpandTools, showRawParameters, showThinking, selectedProject, provider, hideHeader = false }: MessageComponentProps) => {
  ...
  const reasoningContent = stringifyMessageContent(message.reasoning);
  const shouldShowReasoning = showThinking && reasoningContent.trim().length > 0;
  const errorUserHint = message.userHint == null ? '' : String(message.userHint);
  const shouldShowErrorHint = message.type === 'error' && errorUserHint.trim().length > 0;
  const renderMessageBody = (): JSX.Element => {
    const content = String(formattedMessageContent);
              ...
              <div className="text-sm text-gray-700 dark:text-gray-300">
                ...
                {shouldShowErrorHint && (
                  <div className="mt-2 flex items-start gap-2 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 dark:border-amber-800/50 dark:bg-amber-950/30">
                    ...
                    <span className="text-xs text-amber-700 dark:text-amber-300">{errorUserHint}</span>
                  </div>
                )}
              </div>
            )}
