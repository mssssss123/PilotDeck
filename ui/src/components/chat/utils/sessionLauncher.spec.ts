...
describe('sessionLauncher turn identity', () => {
  ...
  it('forwards the optimistic user run id in the command options', () => {
    ...
    startSessionCommand({
      sendMessage,
      selectedProject: { name: '九格智能体平台', path: '/workspace/PilotDeck' } as Project,
      command: 'Continue.',
      runId: 'run-user-1',
...
describe('dialog model submission', () => {
  it.each([{ mode: 'auto' as const }, { mode: 'model' as const, provider: 'selected', model: 'chosen', reasoning: 0.8, speed: 1 }])('snapshots %j in both new and edited requests', (selection) => {
    const sendMessage = vi.fn();
