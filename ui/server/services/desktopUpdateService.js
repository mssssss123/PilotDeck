...
// HTTP header values must remain ASCII even when the display name is Chinese.
...
export function parseVersionParts(value) {
  const normalized = String(value || '')
    .trim()
    .replace(/^(?:九格智能体平台|9gclaw|pilotdeck)[-_ ]?/i, '')
    .replace(/^desktop[-_ ]?/i, '')
    .replace(/^v/i, '');
...
export function launchDownloadedDesktopUpdate(options = {}) {
  ...
  if (relativeToCache.startsWith('..') || path.isAbsolute(relativeToCache)) {
    const error = new Error('Installer path is outside the 九格智能体平台 update cache.');
  ...
  return {
    ...
    needsRestart: true,
    message: 'Installer launched. Complete the installer flow, then restart 九格智能体平台.',
  };
}
...
async function fetchLatestRelease(options) {
  ...
  if (!release) throw new Error('No 九格智能体平台 desktop releases are available.');
  return release;
}
