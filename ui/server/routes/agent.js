...
router.post('/', validateExternalApiKey, async (req, res) => {
  ...
  // After the 九格智能体平台-only migration any incoming `provider` is just a
  ...
  try {
    ...
    console.log(`🛫 Starting 九格智能体平台 gateway session (provider=${provider})`);
    ...
    if (createBranch || createPR) {
      try {
        ...
        if (createPR) {
          ...
          prBody += '\n\n---\n*This pull request was automatically created by 九格智能体平台 Agent.*';
