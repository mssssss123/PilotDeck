...
/**
 * Map a 九格智能体平台 `WebSessionInfo` onto the legacy `ProjectSession`
...
async function getProjects(progressCallback = null) {
    ...
    // toggle section. 九格智能体平台's gateway.listProjects() only returns
...
async function addProjectManually(projectPath, _displayName = null) {
    ...
    // Materialize a 九格智能体平台 project directory and drop a `.cwd` marker
    // recording the real absolute path. We need the marker because
    // createProjectId() encodes both '/' and literal '-' to '-', so the
    // 九格智能体平台's listWebProjects() heuristically tries each `-` as a
    ...
    try {
        ...
        console.warn(
            `[projects] failed to materialize 九格智能体平台 project dir for ${name}:`,
            error?.message || error,
        );
    }
...
async function renameProject(_projectName, _displayName) {
    // 九格智能体平台 does not yet expose a rename API. Display names are derived
