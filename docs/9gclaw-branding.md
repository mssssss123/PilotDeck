...
- 当前功能基线：`OpenBMB/PilotDeck` 的 2026-09-15 提交 `2df7127d`。两次品牌提交已 rebase 到该主线，包含上游最新功能与修复。原始品牌基线为 2026-09-05 的 `8ba2eb04`。
...
安装器默认下载此 fork 的 `feat/9gclaw` 分支。更新逻辑沿用最新主线的统一 Release 机制，默认读取此 fork 的 Releases。开发模式、非 main 分支和 worktree 继续受上游的自动更新资格限制；本分支实例通过手动构建部署更新，不会自动切回上游版本。
