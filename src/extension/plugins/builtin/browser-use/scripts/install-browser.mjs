...
function findRuntimeRoot() {
  ...
  throw new Error("九格智能体平台 runtime root not found. Set PILOTDECK_RUNTIME_ROOT and retry.");
}

function assertRuntimeRoot(candidate, source) {
  if (isRuntimeRoot(candidate)) return candidate;
  throw new Error(
    `九格智能体平台 runtime root from ${source} is incomplete: ${candidate}. ` +
...
function resolveBrowsersPath(runtimeRoot) {
  ...
  if (process.env.APPDATA) {
    return resolve(process.env.APPDATA, "九格智能体平台", "playwright-browsers");
  }
