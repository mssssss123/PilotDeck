...
export class WeComChannel implements ChannelAdapter {
  ...
  private isDmAllowed(senderId: string): boolean {
    ...
    this.logger?.warn?.("wecom: dm_policy=pairing is not supported in 九格智能体平台; DM ignored");
    return false;
  }
  ...
  private isGroupAllowed(chatId: string, senderId: string): boolean {
    ...
    if (this.groupPolicy === "pairing") {
      this.logger?.warn?.("wecom: group_policy=pairing is not supported in 九格智能体平台; group message ignored");
      return false;
    }
