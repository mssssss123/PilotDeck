...
export function resolvePilotDeckWorkspacePath(
  ...
  if (!root) {
    ...
    return {
      ok: false,
      error: toolError("path_not_allowed", `Path ${inputPath} is outside the 九格智能体平台 workspace.`),
    };
  }
  ...
  if (options?.mustExist) {
    ...
    if (!isPathWithinRoot(real, realRoot)) {
      return {
        ok: false,
        error: toolError("path_not_allowed", `Path ${inputPath} resolves outside the 九格智能体平台 workspace.`),
      };
    }
  }
