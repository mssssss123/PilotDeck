...
const BASH_TOOL_DESCRIPTION = `Run a shell command in the 九格智能体平台 workspace.

Usage:
- The \`command\` parameter is passed to 九格智能体平台's resolved command shell (Bash preferred; Unix falls back to \`/bin/sh\`, Windows falls back to \`cmd.exe\` and then PowerShell 7).
...
- Do not use shell-level backgrounding (\`nohup\`, \`disown\`, \`setsid\`, trailing \`&\`) in bash. Use task_create so 九格智能体平台 can track lifecycle and output.
  ...
  return {
    ...
    inputSchema: {
      ...
      properties: {
        command: {
          type: "string",
          description: "The shell command to execute (passed to 九格智能体平台's resolved command shell).",
        },
        timeout: {
          type: "integer",
