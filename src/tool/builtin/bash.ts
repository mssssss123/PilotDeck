...
Usage:
- The \`command\` parameter is passed to 9GClaw's resolved command shell (Bash preferred; Unix falls back to \`/bin/sh\`, Windows falls back to \`cmd.exe\` and then PowerShell 7).
...
- Do not use shell-level backgrounding (\`nohup\`, \`disown\`, \`setsid\`, trailing \`&\`) in bash. Use task_create so 9GClaw can track lifecycle and output.
  ...
  return {
    ...
    inputSchema: {
      ...
      properties: {
        command: {
          type: "string",
          description: "The shell command to execute (passed to 9GClaw's resolved command shell).",
        },
        timeout: {
          type: "integer",
