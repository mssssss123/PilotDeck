 ...
 * by an `McpRuntime`) into 九格智能体平台 `ToolDefinition`s suitable for
 ...
 *          reflected onto the 九格智能体平台 tool flags so the permission
 *          engine can decide whether to ask.
 *
 * Result transformation (M14): MCP ContentBlock types `text` and `image`
 * are mapped to their 九格智能体平台 equivalents so that images (e.g. Playwright
