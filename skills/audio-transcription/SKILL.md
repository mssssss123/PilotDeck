...
九格智能体平台 provides this Skill itself; there is no Skill file to install. First check whether `mcp__funasr__transcribe_audio` is available. If its local runtime or models are missing, run the exact `npm --prefix "..." run install:asr` command shown by the attachment or MCP diagnostic. It points at the 九格智能体平台 source checkout or installed app directory:

```bash
npm --prefix "<九格智能体平台 directory>" run install:asr
```
